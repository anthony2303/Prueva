...
handler.all = async function(m, {conn}) {
  ...
  if (/^hola$/i.test(m.text) && !chat.isBanned) {
    ...
    this.conn.sendPresenceUpdate('recording', m.chat);
    this.conn.sendMessage(m.chat, {audio: {url: vn}, fileName: 'error.mp3', mimetype: 'audio/mpeg', ptt: true}, {quoted: m});
  }
  ...
  if (!chat.isBanned && m.text.match(/(patito)/gi)) {
    if (!db.data.chats[m.chat].audios) return;
    if (!db.data.settings[mconn.conn.user.jid].audios_bot && !m.isGroup) return;
    const vn = './media/patito.mp3';
    this.conn.sendPresenceUpdate('recording', m.chat);
    this.conn.sendMessage(m.chat, {audio: {url: vn}, fileName: 'error.mp3', mimetype: 'audio/mpeg', ptt: true}, {quoted: m});
  }

  if (!chat.isBanned && m.text.match(/(levantense)/gi)) {
    if (!db.data.chats[m.chat].audios) return;
    if (!db.data.settings[mconn.conn.user.jid].audios_bot && !m.isGroup) return;
    const vn = './media/levantense.mp3';
    this.conn.sendPresenceUpdate('recording', m.chat);
    this.conn.sendMessage(m.chat, {audio: {url: vn}, fileName: 'error.mp3', mimetype: 'audio/mpeg', ptt: true}, {quoted: m});
  }

  if (!chat.isBanned && m.text.match(/(@56941827124|@56921994449)/gi)) {
    if (!db.data.chats[m.chat].audios) return;
    if (!db.data.settings[mconn.conn.user.jid].audios_bot && !m.isGroup) return;
    const vn = './media/geri.mp3';
    this.conn.sendPresenceUpdate('recording', m.chat);
    this.conn.sendMessage(m.chat, {audio: {url: vn}, fileName: 'error.mp3', mimetype: 'audio/mpeg', ptt: true}, {quoted: m});
  }
